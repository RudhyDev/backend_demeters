...
import { DeleteVendorUseCase } from './delete-vendor.use-case';
...
describe('DeleteVendorUseCase', () => {
  ...
  beforeEach(async () => {
    ...
    useCase = module.get<DeleteVendorUseCase>(DeleteVendorUseCase);
  });
  ...
  it('deve excluir uma loja com sucesso', async () => {
    ...
    vendorRepository.delete.mockResolvedValue();

    // Act
    await useCase.execute(vendorId);

    // Assert
    expect(vendorRepository.findById).toHaveBeenCalledWith(vendorId);
    expect(vendorRepository.delete).toHaveBeenCalledWith(vendorId);
  });
  ...
  it('deve lançar VendorNotFoundException quando a loja não é encontrada', async () => {
    ...
    expect(vendorRepository.findById).toHaveBeenCalledWith(vendorId);
    expect(vendorRepository.delete).not.toHaveBeenCalled();
  });
});
